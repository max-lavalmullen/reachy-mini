...
from reachy_mini_rubik_coach_app.camera_worker import CameraWorker


def _default_head_tracker() -> str:
    raw = (os.getenv("REACHY_MINI_HEAD_TRACKER", "rubiks_cube") or "rubiks_cube").strip()
    return raw or "rubiks_cube"


def parse_args() -> Tuple[argparse.Namespace, list]:  # type: ignore
    """Parse command line arguments."""
    parser = argparse.ArgumentParser("Reachy Mini Rubik Coach App")
    parser.add_argument(
        "--head-tracker",
        choices=["yolo", "mediapipe", "rubiks_cube", None],
        default=_default_head_tracker(),
        help="Choose head tracker (default: REACHY_MINI_HEAD_TRACKER or rubiks_cube)",
    )
    ...
    if not args.no_camera:
        ...
        if args.head_tracker is not None:
            if args.head_tracker == "yolo":
                from reachy_mini_rubik_coach_app.vision.yolo_head_tracker import HeadTracker
                ...
                head_tracker = HeadTracker()
            elif args.head_tracker == "rubiks_cube":
        ...
        if args.local_vision:
            try:
                from reachy_mini_rubik_coach_app.vision.processors import initialize_vision_manager
