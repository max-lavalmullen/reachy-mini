...
from reachy_mini_rubik_coach_app.config import DEFAULT_PROFILES_DIRECTORY as DEFAULT_PROFILES_PATH  # noqa: F401

# Import config to ensure .env is loaded before reading REACHY_MINI_CUSTOM_PROFILE
from reachy_mini_rubik_coach_app.config import config  # noqa: F401
from reachy_mini_rubik_coach_app.tools.tool_constants import SystemTool


if TYPE_CHECKING:
    from reachy_mini_rubik_coach_app.tools.background_tool_manager import BackgroundToolManager


logger = logging.getLogger(__name__)


DEFAULT_PROFILES_MODULE = "reachy_mini_rubik_coach_app.profiles"
...
def _load_profile_tools() -> None:
    ...
    for tool_name in tool_names:
        ...
        if not loaded:
            shared_module_path = f"reachy_mini_rubik_coach_app.tools.{tool_name}"
            try:
                source = _try_load_tool(
                    tool_name,
